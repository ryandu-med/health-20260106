...
CACHE_FILE = os.path.join(BASE_DIR, 'search_index.pkl')  # <--- 新增：缓存文件路径
...
def get_sorted_items(path):
    """文件/文件夹排序：确保 1, 2, 3 或 一, 二, 三 顺序正确"""
...
def main_app():
    ...
    with st.sidebar:
        ...
        st.markdown("<hr>", unsafe_allow_html=True)
        # 增加一个刷新缓存的按钮（只有管理员可见），防止加了新文件搜不到
